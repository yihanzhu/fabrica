...
The v2 registry also carries one complete, unselected evidence-identity correction.
It requires every passed evidence item to come from the exact selected execution;
incident mismatches remain valid only when all evidence is non-passing. The stable
wrapper still selects the prior v2 generation until a separate atomic cutover binds
the corrective generation's reviewed merge and publisher receipt.
