...
portable_core_ingress_analyze() {
  ...
  if "$PORTABLE_CORE_INGRESS_OD" -An -v -t u1 "$raw_path" 2>/dev/null |
      LC_ALL=C "$PORTABLE_CORE_INGRESS_AWK" \
        ...
        function flush_token_chunk() {
          ...
          printf "%s", token_chunk >> scalar_file
          if (token_role == "key") printf "%s", token_chunk >> key_file
          token_chunk = ""
        }
        ...
        END {
          ...
          if (runtime_bad) {
            ...
            print "S", structurally_canonical, max_depth
          }
        }
  ...
  if "$PORTABLE_CORE_INGRESS_JQ" -n --stream --stream-errors -j '
      foreach (inputs, {end:true}) as $event
        ({emit:"", parse:true, runtime:true};
         ...
         .emit)
    ' "$scalar_path" 2>/dev/null > "$scalar_canonical_path"; then
    :
  else
    portable_core_ingress_error E_RUNTIME
    return 1
  fi
  if "$PORTABLE_CORE_INGRESS_OD" -An -v -t u1 "$scalar_canonical_path" 2>/dev/null |
      "$PORTABLE_CORE_INGRESS_AWK" '
        {
          for (i = 1; i <= NF; i++) if (($i + 0) == 0) found = 1
        }
        END { print found ? "P" : "C" }
      ' > "$scalar_marker_path" 2>/dev/null; then
    pipeline_status=("${PIPESTATUS[@]}")
  else
    pipeline_status=("${PIPESTATUS[@]}")
  fi
  ...
  if [ -s "$key_pair_path" ]; then
    if "$PORTABLE_CORE_INGRESS_JQ" -n -j \
      ...
      ' "$key_pair_path" 2>/dev/null > "$key_order_path"; then
      :
    else
      portable_core_ingress_error E_RUNTIME
      return 1
