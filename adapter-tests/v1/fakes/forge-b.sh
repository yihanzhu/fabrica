...
/usr/bin/printf '%s' "$content" > "$candidate_root/result.pending"
/bin/mv "$candidate_root/result.pending" "$candidate_root/result.txt"
...
"$jq_bin" -S -c -n --arg commit "$commit_id" --arg tree "$tree_id" --arg object "$object_id" '
  {commit_id:$commit,file_object_id:$object,package_id:"fake.forge.b",phase:"forge",
