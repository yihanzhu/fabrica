...
- Once the user explicitly approves an issue, run the single launch flow: apply the **`ready`** label as the record of that approval. **What happens next depends on the repo's configured launch mode** — and exactly one of the two modes is wired, so there is **exactly one coder launch per approved issue** (no duplicate branches/PRs):
  - **Faber-driven mode (the live default — no coder routine wired):** applying `ready` is *your own cue* to **immediately spawn a Claude coder subagent** to implement the issue and open a PR (label `round-0`). The spawn is a manual in-session action, not an automated trigger.
  - **Routine mode (the optional `issues.labeled` coder routine is wired instead):** the `ready` label **fires that routine**, which launches the coder. In this mode you must **NOT** spawn the coder yourself — doing so would double-launch. Just apply `ready` and let the routine handle it.
- **One active launch path.** The two modes above are **mutually exclusive — never both wired.** Confirm which one this repo uses before acting: if a Claude Coder *routine* on `issues.labeled` is connected, it owns the launch and you don't spawn; if no routine is connected (the live default), you spawn. Wire one mechanism only.
