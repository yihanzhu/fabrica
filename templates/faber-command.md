...
- **One active launch path.** Spawning the coder yourself (above) is the **active** path. A Claude Coder *routine* wired to `issues.labeled` is an **optional, mutually-exclusive** alternative — **never both.** If such a routine is connected, the `ready` label fires it, so you must NOT also spawn the coder; if no routine is connected (the live default), you spawn it. Wire one mechanism only.
