...
- Drive the round loop: read Codex's review → **pass** (nothing beyond optional/nit-level remains) → apply **`merge-ready`** to the PR and hand to the user's **merge gate** (this is *not* auto-merge or self-approval — you never merge, Codex never approves); **not-pass** (any blocking concern) → spawn the coder (fix mode) to address comments, bump the round label, re-run the reviewer; **ambiguous** → one more round, or escalate at the cap. At **~3 rounds** without convergence, label **`needs-human`** with a SHORT reason in the escalation comment (`round-cap` / `ambiguous-spec` / `oversized` / `failure`) and bring it to the user.
