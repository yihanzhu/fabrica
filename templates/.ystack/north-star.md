...
star lives** — at `.ystack/north-star.md`, in your repo, owned by your repo (not ystack's).
It is the foundation for per-target steering: ystack's tooling reads your north star from
here, so running ystack against your repo debates proposals against *your* goal rather than
a shared one.

**ystack's tooling uses this file.** You create it yourself: copy this shipped template into
your repo as `.ystack/north-star.md`, replace the placeholder, remove the
`<!-- ystack-shipped-default -->` marker, then commit it (`scripts/setup-target-repo.sh` only
...
**This is a placeholder — replace it.** The entry below is ystack's shipped default, carrying
the stable marker `<!-- ystack-shipped-default -->`. **Write your own north star, then remove
that marker from the active heading line, and commit the file.** The marker lets ystack's
...
### Placeholder — replace with your own goal  ·  status: **active** · <!-- ystack-shipped-default --> shipped default: this is a placeholder, not your project's goal — write your own north star below and remove the `ystack-shipped-default` marker on this line when you do.
...
### Vetoed-but-yshifu-thought-relevant (manager-debate filtered these out)

When the manager-reviewer vetoes a proactive issue yshifu believed was genuinely
north-star-relevant, yshifu logs it here (default-drop, but surfaced) so you can see what
